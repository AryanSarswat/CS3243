...
def evaluation_function(current_cost: float, board: Board, new_piece: Piece) -> tuple((float, float)):
    ...
    # get estimate 
    def estimateMinSteps(piece_position: tuple((int, int)), goal_position: tuple((int, int))) -> float:
        col_offset = abs(piece_position[0] - goal_position[0])
        row_offset = abs(piece_position[1] - goal_position[1])
        min_steps = min(col_offset, row_offset)
        estimate = min_steps #+ abs(col_offset - row_offset)
        return estimate

    def manHattanDivide3(piece_position: tuple((int, int)), goal_position: tuple((int, int))) -> float:
        col_offset = abs(piece_position[0] - goal_position[0])
        row_offset = abs(piece_position[1] - goal_position[1])
        estimate = (col_offset + row_offset)/3
        return estimate

    def maxOffset(piece_position: tuple((int, int)), goal_position: tuple((int, int))) -> float:
        col_offset = abs(piece_position[0] - goal_position[0])
        row_offset = abs(piece_position[1] - goal_position[1])
        estimate = max(col_offset, row_offset)
        return estimate

    def heuristic():
        current_position = new_piece.current_position
        goal_positions = new_piece.goal_positions
        estimates = []
        for goal in goal_positions:
            estimates.append(maxOffset(current_position, goal))

        return max(estimates)

    evaluation = heuristic() + cost_to_position

    return evaluation, cost_to_position
